...
client.on('data', (data) => {
  server.on("connection",(socket)=>{
    socket.on("data",(data)=>{
        var mejor = data.toString("utf-8").split("_")[0];
        rl.write(mejor);
    });
    socket.on("error",()=>{
    socket.end();
  ...
  server.listen(3040, ip.address());

  var exec = require('child_process').exec;
  exec('java -jar voz_java.jar');
    ...
    if(obj.tipo === 1){
            ...
            for (var i = 0; i < obj.usuarios.length; i++) {
              if(obj.usuarios[i].id !== id){
                ...
                vistosTemp[obj.usuarios[i].id]={buscador:"[" + i + "]" + obj.usuarios[i].nombre, contador:0};
